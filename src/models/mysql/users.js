...
export class userModel {
    ...
    static async create({ input }) {
        ...
        try{
           ...
           await connection.query(`INSERT INTO users (id, nombre, email, passw)
          VALUES (UUID_TO_BIN("${uuid}"), ?, ?,?);`,[nombre,email,contraseña])
        ...
        const [user] = await connection.query("SELECT BIN_TO_UUID(id),nombre,email,passw FROM users WHERE BIN_TO_UUID(id) = ?;",[uuid]) 
       return user
    ...
    static async delete({id}){
        ...
        try{
            ...
            await connection.query("DELETE FROM users WHERE BIN_TO_UUID(id) = ?;",[id]) 
    ...
    static async update({ id, input }){
        ...
        const fields = Object.keys(updateFields).map(field => `${field} = ?`).join(', ');// items o campos a cambiar
       ...
       console.log(fields)
        try {
            ...
            console.error(e);
            return { success: false, message: "An error occurred", error: e.sqlMessage }; 
        }
